  ...
  return (
    <div className="group relative bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col h-full border border-gray-200">
      ...
      <div className="p-3 flex flex-col flex-grow">
        ...
        <p className="text-[10px] text-gray-500 mb-1">ID: {product.productId}</p>
        ...
        <div className="flex items-center mb-2">
          <div className="flex text-yellow-400 mr-1">
            {[...Array(5)].map((_, i) => (
              <svg
                key={i}
                className={`w-2.5 h-2.5 ${i < (product.rating || 4) ? 'fill-current' : 'fill-gray-300'}`}
                viewBox="0 0 20 20"
              >
        ...
        <div className="flex justify-center mt-2 pt-2 border-t border-gray-100">
          <Link 
            ...
            className="w-full text-center bg-yellow-500 hover:bg-yellow-400 text-gray-900 font-medium py-2.5 px-5 rounded transition-colors duration-300 text-xs"
          >
            View Details
          </Link>
        </div>
