...
export default function ProductOverview() {
          ...
          const productData = {
            ...
            reviews: Array.isArray(res.data.reviews) 
              ? res.data.reviews.map(review => ({
                  _id: review._id || Math.random().toString(36).substring(2, 9),
                  email: review.email || 'anonymous@user.com',
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        setProduct({
          ...
          reviews: Array.isArray(productResponse.data.reviews) 
  ...
  const renderReviews = () => {
    ...
    return product.reviews.map((review) => {
      ...
      return (
        <div key={review._id} className="border-t pt-4 pb-2">
          ...
          <p className="text-sm text-gray-500">
            By {(review.email || 'anonymous').split('@')[0]} • {review.date ? new Date(review.date).toLocaleDateString() : 'unknown date'}
          </p>
        </div>
      );
    });
                ...
                {!canReview && (
                  <p className="text-gray-500 mb-4">
                    {getCurrentUserEmail() 
                      ...
                      : "Please login to check review eligibility"}
                  </p>
                )}
